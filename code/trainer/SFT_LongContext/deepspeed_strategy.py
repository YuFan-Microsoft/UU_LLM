...
from utils import DistributedSampler, get_ring_attn_group, set_ring_attn_group


# ---------------------------------------------------------------------------
# DeepSpeed config helpers (formerly deepspeed_utils.py)
# ---------------------------------------------------------------------------
...
def get_train_ds_config(
    ...
    ds_config = {
        ...
        "checkpoint": {"load_universal": use_ds_universal_ckpt},
        "compile": {"deepcompile": deepcompile},
        "tensor_parallel": {"autotp_size": tensor_parallel_size},
    }
...
def torch_dist_barrier_and_cuda_sync():
    """Barrier across distributed processes and synchronize CUDA ops."""
...
class DeepspeedStrategy(ABC):
    ...
    def setup_ring_attn(self, ds_device_mesh):
        ...
        from utils import patch_transformers_for_ring_flash_attn
    ...
    def _get_model_parameters(self, model: nn.Module, *, optim: Optional[str] = None, weight_decay: float = 0.0):
        ...
        if optim is None:
            optim = self.optim
        raw_model = model.model if isinstance(model, LMModel) else model
        if optim == "muon":
        ...
        * ``model`` — evaluation only, no optimizer.
        ...
        for arg in args:
            assert isinstance(arg, tuple) and len(arg) == 2, \
                f"prepare() expects (model, cfg) tuples; got {type(arg).__name__}"
            model, cfg = arg
            if model is None:
    ...
    def _ds_init_train_model(self, model, cfg: dict):
        ...
        is_lm_model = isinstance(model, LMModel)
        ds_config = self.get_ds_train_config(optim_dict=optim_dict, max_norm=cfg.get("max_norm", 1.0))

        if self.ds_tensor_parallel_size > 1:
            tp_model = deepspeed.tp_model_init(
                model=model.model if is_lm_model else model, tp_size=self.ds_tensor_parallel_size, dtype=torch.bfloat16
            )
            if is_lm_model:
        ...
        # Infer optim kind from the DS type (per-model override possible).
        ...
        # This works for LLaMA/Mistral/Qwen base models. Edge cases (GPT-2 wte/wpe, RM
        # value heads) are classified to Muon by the default rule
    ...
    def get_ds_train_config(
        self, optim_dict: Optional[dict] = None, *, max_norm: Optional[float] = None
    ):
        # ``optim_dict`` is None when called from HF model-loading paths (e.g.
        # ``LMModel.from_pretrained``) that only need the ZeRO + bf16 parts — the
